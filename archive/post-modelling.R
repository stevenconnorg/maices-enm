...
PostBioApply <-function(sp.n) {
  ...
  #  
  #   proj_current<-load(paste0(dir_bm,"/",sp.n,"/proj_current/",sp.n,".current.ensemble.projection.out"))
  # myBiomodProj<-get(paste0(sp.n,".current.ensemble.projection.out"))
  # myBiomod_raster <- get_predictions(myBiomodProj)
  # KAPPAem<-paste0(sp.n,"_EMwmeanByKAPPA_MAXENT.Phillips_mergedRun_mergedData")
  # TSSem<-paste0(sp.n,"_EMwmeanByTSS_MAXENT.Phillips_mergedRun_mergedData")
  # ROCem<-paste0(sp.n,"_EMwmeanByROC_MAXENT.Phillips_mergedRun_mergedData")
  ...
  # KAPPA<-paste0("result.",sp.n,"_EMwmeanByKAPPA_MAXENT.Phillips_mergedRun_mergedData")
  # TSS<-paste0("result.",sp.n,"_EMwmeanByTSS_MAXENT.Phillips_mergedRun_mergedData")
  # ROC<-paste0("result.",sp.n,"_EMwmeanByROC_MAXENT.Phillips_mergedRun_mergedData")
  ...
  # 
  # 
  # KAPPA_AUC <- performance( ROC_curve_KAPPA, "auc" )@y.values
  # TSS_AUC <- performance( ROC_curve_TSS, "auc" )@y.values
  # ROC_AUC <- performance( ROC_curve_ROC, "auc" )@y.values
  # 
  ...
  #                              KAPPA_acc,TSS_acc,ROC_acc,
  #                              KAPPA_err,TSS_err,ROC_err,
  #                              KAPPA_fpr,TSS_fpr,ROC_fpr,
  #                              KAPPA_fnr,TSS_fnr,ROC_fnr,
  #                              KAPPA_f,TSS_f,ROC_f
  #                              )
  #                    
  #                    )
  # eval.df$variety<-sp.n
  # View(eval.df)
  # rownames(AUC.df)<-1:nrow(AUC.df)
  # class(AUC.df)
  # AUC.df[,2]<-as.numeric(AUC.df[,2])
  # write.csv(AUC.df,file=paste0(dir_out,"/ensemble-evals/",sp.n,"-ensemble-AUC.csv"))
  # 
...
# 
#     avg_wmean_bin_grd<-paste0(dir_binary,"/",sp.n,"_",p,"_85_thresh_bin-avg-wmean.grd")
...
#   
#   dir_wmean<-paste0(dir_out,"/wmean-consensus")
#   wmean<-Sys.glob(paste0(dir_wmean,"/",sp.n,"*.grd")) # get ensemble files out
...
#                   xlab="X", ylab="Y",main=paste0(sp.n," \nProjected Relative Probability of Presence"), names.attr=c("1970-2000"# ,"2041-2060","2061-2080"
#                   )
#   )
#   
#   png(paste0(dir_wmean,"/",sp.n,"-current-wmean-em-projs.png"), width = 10, height = 10, units = 'in', res = 600)
#   print(plt  )
#   dev.off()
#   
#   # bin_grds<-Sys.glob(paste0(dir_binary,"/",sp.n,"*.grd"))
...
#   # 
#   # binplt <-levelplot(binstack, par.setqtings=mapTheme,xlab="X", ylab="Y",main=paste0(sp.n," Binary Predictions (.75 threshold) \nfrom Average Weighted Mean Ensemble"),maxpixels= 20000000, names.attr=c("1970-2000","2041-2060","2061-2080"))
#   # 
#   # png(paste0(dir_binary,"/",sp.n,"-wmean-bins.png"), width = 10, height = 10, units = 'in', res = 600)
#   # print(binplt )
#   # dev.off()
#   
#   
...
#   chg<-Sys.glob(paste0(dir_rangeras,"/",sp.n,"*.grd")) # get ensemble files out
#   chgras<-raster(chg[1])
...
#   grid.newpage()
#   
#   ggsave(paste0(sp.n,"_change_by_pixels.png"),width=10, height=10, units="in", plot = grid.draw(g), device = "png",dpi=600, path = dir_rangeras,
#          scale = 1)
#   
#   
#   #} # tryCatch
...
# clusterEvalQ(cl, library(gtable))
# clusterEvalQ(cl, library(grid))
# clusterEvalQ(cl, library(ggplot2))
# clusterEvalQ(cl, library(rasterVis))
# clusterEvalQ(cl, library(sp))
# 
# 
# clusterExport(cl=cl, list("sp.n","projects","models","metrics","PostBioApply",
#                           "dir_out","dir_R","dir_dat","dir_stacks","dir_bm","root"))
# 
# #for (sp.n in sp.n){
# #print(sp.n)
# parLapply(cl, sp.n,PostBioApply)
# #}
# stopCluster(cl)#######
...
for (p in projs){
  ...
  bin<- raster(paste(dir_bm,"/",sp.n[1],"/proj_",p,"/proj_",p,"_",sp.n[1],"_ensemble_",metric,"bin.grd",sep=""))
  for(metric in metrics){
    for(sp in sp.n[-1]){
      ras<-raster(paste(dir_bm,"/",sp,"/proj_",p,"/proj_",p,"_",sp,"_ensemble_",metric,"bin.grd",sep=""))
      
      ras<-resample(ras,bin,method="ngb")
...
# df$Presenc
# df$Presenc<-factor(df$Presenc,levels=c("Sin población indígena","Población indígena dispersa","Población con presencia indígena","Población indígena"),order=TRUE)
# 
# 
# 
