...
##' @param parallel        logical. If TRUE, the parallel computing is enabled (highly recommended)
##' @param number.cores    numeric. Number of cores used for parallel computing. (Default: detectCores()-1)
##' 
##' @references 
...
##' }
        ...
        BIOMOD_tuning <- function(data,
                                  models = c('GLM','GBM','GAM','CTA','ANN','FDA','MARS','RF','MAXENT.Phillips','MAXENT.Tsuruoka'),
  ...
  packages <- NULL
  if(sum(c('GLM','GBM','GAM','CTA','ANN','FDA','MARS','RF','MAXENT.Phillips','SRE') %in% models)>0){if(!require(caret)){stop("caret package required!")};packages<-c(packages,"caret"); if(is.null(trControl)){trControl <- trainControl(method="cv",summaryFunction=twoClassSummary,classProbs=T, returnData = F)}}
  if("MAXENT.Phillips" %in% models){if(!require(ENMeval)){stop("ENMeval package required!")};packages<-c(packages,"ENMeval")}  
  if("MAXENT.Tsuruoka" %in% models){if(!require(maxent)){stop("maxent package required!")};packages<-c(packages,"maxent")}  
    ...
    if('SRE' %in% models){
      ...
      for (rep in 1:trControl$repeats){
        ...
        for (i in 1:trControl$number) {
          ...
          tune.SRE <- rbind(tune.SRE,cbind(presence.absence.accuracy(DATA, threshold=as.vector(optimal.thresholds(DATA,opt.methods=3)[2],mode="numeric")),quant))
        }
      }}
  ...
  if('MAXENT.Phillips' %in% models){
    ...
    try(tune.MAXENT.Phillips <- tuning(pres=data@data.env.var[data@data.species==1 & !is.na(data@data.species),],
...
#### Modified tuning function from the ENMeval package to tune MAXENT.Phillips 
...
### Specify dismo!!! Problems with biomod!
        ...
        ########
            ...
            if (parallel == TRUE) {
              ...
              out <- foreach(i = seq_len(length(maxent.args)), .packages = c("dismo", 
