...
load(file=paste0(dir_div,"/pvclust-var-imp-ward2.RData"))
...
varimp.w.group<-read.csv(paste0(dir_div,"/var-imp-wgrp5.csv"))

varimp.w.group<-varimp.w.group[,-1]
varimp.w.group<-varimp.w.group[,-1]
...
varimpmeans<-varimpmeans[,13:20]
varimpsds<-varimpsds[,13:20]
...
barCenters<-barplot(as.matrix(varimpmeans),ylim=c(0,max(varimpmeans)+max(varimpsds)), main="Variable Importance by Landrace Cluster", ylab="Importance (0-1)",
 	legend = rownames(varimpmeans), beside=TRUE)
...
arrows(barCenters, as.matrix(varimpmeans - varimpsds), barCenters,
...
chg.files <- list.files(pattern="*f50*")
...
percchang<-rangesorted %>% filter(time == "2041-2060")

percchang<-percchang[,c("species","time","SpeciesRangeChange","PercLoss","PercGain","CurrentRangeSize","FutureRangeSize.NoDisp","FutureRangeSize.FullDisp")]
View(percchang)

colnames(percchang)[1]<-"LandRace"
colnames(percchang)[2]<-"Time-period"
...
write.csv(df.by.grp, file = paste0(dir_out,"/alpha-div-mean-by-grp-time.csv"))
df.by.grp<-df
...
# df.by.grp$chg<-chg
colname<-colnames(df.by.grp[i])
newname<-sub("_mean_div","mnd",colname)
newname<-sub("Group_","G",newname)

newname<-sub("_proj","",newname)
newname<-sub("_rcp85","",newname)

colnames(df.by.grp)[i] <- newname
}
...
library(rgdal)
poly <- readOGR("E:\\thesis\\01_data\\ind\\presindigw\\presindigw.shp",layer="presindigw")
dfj<-read.csv(paste0(dir_out,"/alpha-div-mean-by-grp-time.csv"))
df<-dfj

library(stringr)
dfj$MUN_OFICIA=str_pad(dfj$MUN_OFICIA, 5, pad = "0")
poly@data$MUN_OFICIA=str_pad(dfj$MUN_OFICIA, 5, pad = "0")

poly@data$MUN_OFICIA<-as.character(poly@data$MUN_OFICIA)
dfj$MUN_OFICIA<-as.character(dfj$MUN_OFICIA)

head(poly@data)
head(dfj)

# poly@data<-merge(poly@data,dfj,by="MUN_OFICIA")

poly@data = data.frame(poly@data, dfj[match(poly@data[,"MUN_OFICIA"], dfj[,"MUN_OFICIA"]),])
View(poly@data)



writeOGR(poly,"E:\\thesis\\01_data\\ind\\munidiv_time.shp",layer="munidiv_time", driver="ESRI Shapefile")
write.csv(dfj,paste0(dir_out,"/alpha-div-mean-by-grp-time.csv"))
df<-read.csv(paste0(dir_out,"/alpha-div-mean-by-grp-time.csv"))


View(poly@data)

poly@data$G2_50chgp<-((poly@data$G2_50mnd-poly@data$G2_currentmnd)/poly@data$G2_currentmnd)

poly2 <- sp.na.omit(poly)   

View(poly2)
writeOGR(obj=poly , dsn="E:\\thesis\\01_data\\ind\\munidiv_time.shp", layer="munidiv_time", driver="ESRI Shapefile") 
plot(presind)
...
# df$Presencia<-factor(df$Presencia,levels=c("Sin población indígena","Población indígena dispersa","Población con presencia indígena","Población indígena"),order=TRUE)
...
ghm<-games.howell(df$marmun, df$med.div)

write.csv(ghm,paste0(dir_out,"/ind/games_howell_med-div_by_marginality.csv"))


colnames(df$games.howell)

gh<-games.howell(df$Presencia, df$med.div)
ggplot(df, aes(x=Presencia, y=med.div, fill=Presencia)) + geom_boxplot()


write.csv(gh,paste0(dir_out,"/ind/games_howell_med-div_by_indpres2.csv"))
gh<-posthocTGH(df$med.div, df$Presencia, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)
str(gh)

ghm<-posthocTGH(df$med.div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)


write.csv(gh$output$games.howell,paste0(dir_out,"/ind/games_howell_med-div_by_marginality.csv"))
...
library(ggplot2)
ggplot(df, aes(x=Presencia, y=Group_2_mean_div, fill=Presencia)) + geom_boxplot()
...
gh1m<-posthocTGH(df$Group_1_mean_div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)

gh2m<-posthocTGH(df$Group_2_mean_div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)

gh3m<-posthocTGH(df$Group_3_mean_div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)

gh4m<-posthocTGH(df$Group_4_mean_div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)

gh5m<-posthocTGH(df$Group_5_mean_div, df$marmun, method=c("games-howell"),
           conf.level = 0.95, digits=2, p.adjust="none",
           formatPvalue = TRUE)
