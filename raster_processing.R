...
writeRaster(
  topostack,
  paste0(dir_stacks, "topostack.grd"),
  bylayer = FALSE,
  format = 'raster',
  overwrite = T
)
...
names(f50cropstack)
names(f50cropstack) <-
  layerNames(pres_cropstack)  # apply presmodstack layer names to future stacks
...
env <-
  stack(
    list.files(
      path = "E:\\thesis\\01_data\\envirem\\2061-2080\\",
      pattern = ".tif",
      full.names = T
    )
  )
...
for (cropstack in cropstacks) {
  ...
  writeRaster(
    full_stack,
    paste0(dir_stacks, "/", cropstack@title, "_biostack", ".grd"),
    bylayer = FALSE,
    format = 'raster',
    overwrite = T
  )
