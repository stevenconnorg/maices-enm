...
BiomodProj <- BIOMOD_Projection(
  ...
  print(paste0("Projecting onto 2070 for ",myRespName))
  ...
  myBiomodProjFuture70 <- BIOMOD_Projection(
   ...
   print(paste0("Projecting onto 2050 for ",myRespName))
  ...
  myBiomodEM <- BIOMOD_EnsembleModeling(
    ...
    em.by="algo",
    eval.metric = metrics,
    eval.metric.quality.threshold = c(rep(0.85,length(metrics))),
    prob.mean = T,
    prob.cv = T,
    prob.ci = F,
    prob.median = F,
...
#cluster <- makeCluster(28)

#clusterEvalQ(cluster, library(biomod2)) # load a library
#ClusterExport(cl=cluster, list(ls()))
...
sfInit(parallel=TRUE,cpus=np)

# Export packages to snowfall
sfLibrary('biomod2', character.only=TRUE)
sfExportAll()
#sfExport('sp.n')
#sfExport('presmodstack')
#sfExport('f70modstack')
#sfExport('f50modstack')
#sfExport('pa')
